/**
 * Cell scope panel used in the attach-protocol wizard.
 *
 ...
 *
 * Design intent: keep this panel lightweight — we don't reproduce the full
 * hierarchy tree here. Most invocations come from a single-cell entry point
 * (drawer chip, master-plot tooltip), so the scope panel is usually just a
 * "confirm" gate. For multi-cell flows the search-add affordance is enough
 * to reach project scale.
 */
...
import { fetchCellRecordIndex, type IndexCell } from '@/lib/api';
...
export function ScopeStep({ cellIds, onChange }: ScopeStepProps) {
  const [cells, setCells] = React.useState<IndexCell[] | null>(null);
  ...
  const cellsById = React.useMemo(() => {
    const map = new Map<string, IndexCell>();
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        {cellIds.length === 0
          ? 'No cells selected yet. Add at least one to save.'
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-2">
          <span className="text-xs font-medium text-foreground">Add cells</span>
          {cells && (
        ...
        <div className="max-h-64 overflow-y-auto rounded-md border border-border p-1">
          ...
          {!loadError && !cells && (
            <div className="p-3 text-xs text-muted-foreground">Loading cells…</div>
          )}
