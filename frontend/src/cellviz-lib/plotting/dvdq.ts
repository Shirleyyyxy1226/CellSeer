...
function build3D(datasets: Dataset[]): DvDqFigure {
  ...
  datasets.forEach((dataset, i) => {
    ...
    dataset.cycles.forEach((cycle, idx) => {
      traces.push({
        ...
        hovertemplate: `${dataset.label}<br>Capacity: %{x:.2f} Ah<br>Cycle: %{y}<br>dV/dQ: %{z:.2f} V·h/Ah<extra></extra>`,
      });
    });
  });
  ...
  return {
    ...
    layout: {
      font,
      scene: {
        xaxis: { title: { text: 'Capacity (Ah)', font }, gridcolor: '#e0e0e0', tickfont: font },
        yaxis: { title: { text: 'Cycle', font }, gridcolor: '#e0e0e0', tickfont: font },
        zaxis: { title: { text: 'dV/dQ (V·h/Ah)', font }, gridcolor: '#e0e0e0', tickfont: font },
...
function build2D(datasets: Dataset[], opts: BuildDvDqOpts): DvDqFigure {
  ...
  if (viewMode === 'range') {
    ...
    datasets.forEach((dataset, i) => {
      ...
      rangeTraces.push({
        ...
        hovertemplate: `${dataset.label}<br>Capacity: %{x:.2f} Ah<br>dV/dQ: %{y:.2f} V·h/Ah<extra></extra>`,
        showlegend: true,
        connectgaps: false,
      });
      ...
      peaks.push({
        ...
        hovertemplate: `${dataset.label} peak<br>Capacity: %{x:.2f} Ah<br>dV/dQ: %{y:.2f} V·h/Ah<extra></extra>`,
      });
    });
    ...
    datasets.forEach((dataset, i) => {
      ...
      if (baseline) {
        baselineLines.push({
          ...
          hovertemplate: `${dataset.label} Cycle ${baseline.cycle}<br>Capacity: %{x:.2f} Ah<br>dV/dQ: %{y:.2f} V·h/Ah<extra></extra>`,
        });
      }
      const { index: peakIndex, value: peakValue } = findPeak(cycle.y);
      ...
      lines.push({
        ...
        name: dataset.label,
        line: { color, width: 2 },
        hovertemplate: `${dataset.label}<br>Capacity: %{x:.2f} Ah<br>dV/dQ: %{y:.2f} V·h/Ah<br>Peak: ${peakCapacity.toFixed(2)} Ah, ${peakValue.toFixed(2)} V·h/Ah<extra></extra>`,
      });
      peaks.push({
        x: [peakCapacity],
        y: [peakValue],
  ...
  return {
    ...
    layout: {
      font,
      xaxis: { title: { text: 'Capacity (Ah)', font }, tickfont: font, gridcolor: '#e0e0e0', showgrid: true },
      yaxis: { title: { text: 'dV/dQ (V·h/Ah)', font }, tickfont: font, gridcolor: '#e0e0e0', showgrid: true },
