...
describe('buildDvDqFigure', () => {
  it('builds one scatter3d trace per cycle with capacity-axis label', () => {
    ...
    expect(scene?.xaxis?.title).toMatchObject({ text: 'Capacity (Ah)' });
    expect(scene?.zaxis?.title).toMatchObject({ text: 'dV/dQ (V·h/Ah)' });
  ...
  it('builds baseline 2d traces with capacity-based peak hover', () => {
    ...
    expect(fig.layout.xaxis?.title).toMatchObject({ text: 'Capacity (Ah)' });
    expect(fig.layout.yaxis?.title).toMatchObject({ text: 'dV/dQ (V·h/Ah)' });
  });
