...
function build3D(datasets: Dataset[]): DvDqFigure {
  ...
  datasets.forEach((dataset, i) => {
    ...
    dataset.cycles.forEach((cycle, idx) => {
      traces.push({
        ...
        hovertemplate: `${dataset.label}<br>Capacity: %{x:.2f} mAh<br>Cycle: %{y}<br>dV/dQ: %{z:.4f} V mAh⁻¹<extra></extra>`,
      });
    });
  });
  ...
  return {
    ...
    layout: {
      font,
      scene: {
        xaxis: { title: { text: 'Capacity (mAh)', font }, gridcolor: '#e0e0e0', tickfont: font },
...
function build2D(datasets: Dataset[], opts: BuildDvDqOpts): DvDqFigure {
  ...
  return {
    ...
    layout: {
      font,
      xaxis: { title: { text: 'Capacity (mAh)', font }, tickfont: font, gridcolor: '#e0e0e0', showgrid: true },
