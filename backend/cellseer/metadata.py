...
class CellMetadata(BaseModel):
    ...
    def from_dict(
        ...
        """
        Build from an arbitrary flat dict (e.g. a row from the metadata Excel).
        Maps legacy field names used in the existing CellSeer schema.
        """
        ...
        by_norm = {_norm_key(k): k for k in data.keys()}
        normalised: Dict[str, Any] = {}

        # Explicit key selection has highest priority.
        if primary_key_header:
            selected = _find_value(data, by_norm, {_norm_key(primary_key_header)})
            if selected not in (None, ""):
                normalised["cell_id"] = selected
        if id_no_header:
            selected_id_no = _find_value(data, by_norm, {_norm_key(id_no_header)})
    ...
    def from_excel_row(cls, row: Dict[str, Any]) -> "CellMetadata":
        """Convenience alias for from_dict — same logic, different name for clarity."""
